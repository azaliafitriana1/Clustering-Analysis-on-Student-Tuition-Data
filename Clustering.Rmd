...
```{r}
# 1. Load data
df_raw <- read_excel("C:/Users/Azalia Fitriana B/Downloads/Data Asli simandiri 2024.xlsx")          
df <- df_raw
```
...
```
Penjelasan: Dilakukan reduksi dimensi menggunakan PCA, dapat dilihat jika pada PC5 proporsi varians sudah mencapai 94.5%, sehingga dapat diambil PC5.
...
```
Penjelasan: Melakukan dbscan dengan epsilon sebesar 1.3 dan minimal sampel 3, hasilnya terbentuk 8 cluster valid dan 1 cluster noise. Distribusi anggota tiap cluster tidak merata, mayoritas berada di cluster 1, dan anggota cluster 8 sangat minim.
...
```
Penjelasan: Hasil clustering divisualisasikan, dapat dilihat jika masih banyak cluster yang tumpang tindih satu sama lain.
...
```
Penjelasan: Skor silhouette mencapai 0.36 yang memenuhi standar minimal yaitu 0.3, tetapi untuk BSS/TSS masih jauh dari standar minimal yaitu 0.7. DBSCAN pada dataset ini masih kurang baik memisahkan sesuai karakteristik data.
...
```
Penjelasan: Baik silhouette dan BSS/TSS sama-sama memenuhi standar minimal.
...
```
Penjelasan: Baik silhouette dan BSS/TSS sama-sama buruk, ini tidak dapat dijadikan opsi metode.
...
```
Penjelasan: Meskipun sihouette dan BSS/TSS masih di bawah standar, setidaknya masih lebih baik daripada PCM.
...
```
Penjelasan: MFPCM memiliki evaluasi keseluruhan yang cukup baik, meskipun BSS/TSS masih sedikit di bawah standar.
...
```
Penjelasan: Pada grafik, dapat diketahui jika silhouette score paling tinggi didapat oleh DBSCAN, sedangkan BSS/TSS paling tinggi didapatkan FCM. Saya memutuskan untuk memilih metode FCM, karena meskipun silhouette FCM lebih rendah dari DBSCAN, evaluasi FCM secara keseluruhan merupakan yang paling balance.
...
```
Penjelasan: Dilakukan perhitungan rata-rata tiap variabel pada masing-masing cluster, kemudian masing-masing rata-rata dikalikan dengan bobot yang sudah ditentukan. Kemudian didapatkan masing-masing skor UKT yang dijadikan patokan untuk menentukan nilai UKT.
...
print(ukt_final_scores)
```
Penjelasan: Berdasarkan skor UKT yang didapatkan, kemudian ditentukan nilai UKT berdasarkan skala skor UKT dan aturan batas bawah 500.000, dan batas atas 8.000.000.
...
sum(final_df$UKT_Price, na.rm = TRUE)
```
Penjelasan: Semua nilai UKT tiap mahasiswa dijumlahkan, dan didapatkan total Rp1.298.979.598 sebagai pendapatan kampus dari UKT.
